    ...
    <div class="bg-linear-to-br from-indigo-600 to-purple-700 text-white py-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <x-breadcrumb :items="array_filter([
                ...
                ['name' => 'Invoice Generator', 'url' => route('tools.invoice-generator')],
                $seoPage ? ['name' => $seoPage->h1 ?: $seoPage->slug, 'url' => null] : null,
            ])" :schema="true" />
            <h1 class="text-3xl font-bold mb-1">{{ $seoPage?->h1 ?: 'Invoice Generator' }}</h1>
            @if($seoPage?->intro)
                <p class="text-indigo-200 text-sm">{{ $seoPage->intro }}</p>
            @else
