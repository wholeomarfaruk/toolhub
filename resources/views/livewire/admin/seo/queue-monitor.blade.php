    ...
    <div class="flex-1 w-full bg-white rounded-lg min-h-[80vh]">
        ...
        <div class="space-y-6">
            <div class="p-6 border-b border-gray-100">
                <h2 class="text-xl font-bold text-gray-900">Queue Worker Monitor</h2>
                <p class="text-sm text-gray-500 mt-0.5">Auto-refreshes every 15 seconds. Confirms whether your queue worker (cron/systemd) is actually running.</p>
            </div>

            {{-- Worker Heartbeat --}}
            <div class="px-6">
                @if($workerLikelyRunning)
                    <div class="flex items-center gap-3 bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm px-4 py-3.5 rounded-xl">
                ...
                @else
                    <div class="flex items-center gap-3 bg-amber-50 border border-amber-200 text-amber-800 text-sm px-4 py-3.5 rounded-xl">
            ...
            {{-- Summary cards --}}
            <div class="px-6 grid gap-4 grid-cols-2 md:grid-cols-2">
                <div class="bg-white rounded-xl border border-gray-100 p-5 hover:shadow-sm transition-shadow">
                    <div class="flex items-center gap-3 mb-2">
                        ...
                        <div class="text-xs font-medium text-gray-500 uppercase tracking-wide">Pending Jobs (queued, not yet processed)</div>
                    </div>
                ...
                <div class="bg-white rounded-xl border border-gray-100 p-5 hover:shadow-sm transition-shadow">
                    ...
                    <div class="text-2xl font-bold text-gray-900">{{ number_format($failedCount) }}</div>
                </div>
            </div>

            {{-- Pending Jobs --}}
            <div class="px-6">
                <h3 class="text-sm font-bold text-gray-900 mb-3">Pending Jobs (latest 20)</h3>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50 text-gray-500 text-xs font-semibold uppercase tracking-wider">
                                ...
                                <th class="px-4 py-3 text-left">Queued At</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @forelse($pendingJobs as $job)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-4 py-3 font-mono text-xs text-gray-700">{{ $job['job_class'] }}</td>
                                    <td class="px-4 py-3 text-center text-gray-600">{{ $job['attempts'] }}</td>
                                    <td class="px-4 py-3 text-gray-600">{{ \Carbon\Carbon::createFromTimestamp($job['created_at'])->diffForHumans() }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3">
                                        <div class="flex flex-col items-center justify-center py-10 px-6 text-center">
                                            <div class="w-12 h-12 rounded-full bg-gray-50 flex items-center justify-center mb-2">
                                                <i class="bx bx-inbox text-2xl text-gray-300"></i>
                                            </div>
                                            <p class="text-sm text-gray-400">No pending jobs.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                    ...
                    @if($failedCount > 0)
                        <div class="flex gap-2">
                            <button wire:click="retryAllFailed" wire:confirm="Retry all failed jobs?"
                                    class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg transition-colors">
                                Retry All
                            </button>
                            <button wire:click="clearAllFailed" wire:confirm="Permanently delete all failed job records?"
                                    class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold bg-red-50 hover:bg-red-100 text-red-700 rounded-lg transition-colors">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($failedJobs as $job)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-4 py-3 font-mono text-xs text-gray-700 align-top">{{ $job['job_class'] }}</td>
                                    <td class="px-4 py-3 text-gray-600 whitespace-nowrap align-top">{{ $job['failed_at'] }}</td>
                                    <td class="px-4 py-3 text-xs text-red-600 font-mono whitespace-pre-wrap align-top">{{ $job['exception'] }}</td>
                                ...
                                <tr>
                                    <td colspan="4">
                                        <div class="flex flex-col items-center justify-center py-10 px-6 text-center">
                                            ...
                                            <p class="text-sm text-gray-400">No failed jobs.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
